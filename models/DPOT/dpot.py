...
from .dpot_utils import DPOTConfig, TimeAggregator2D, PatchEmbed2D, Block2D
...
class DPOT(PreTrainedModel):
    ...
    def forward(
        ...
        # Neptune inputs - B x T x C x H x W x D
        x=input_data
...
class DPOTNet2D(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(
                1, config.latent_channels, self.patch_embed.out_size[0], self.patch_embed.out_size[1]
            )
        )
        ...
        self.time_agg_layer = TimeAggregator2D(
