...
class AK_Prop(nn.Module):
    def __init__(self, 
        ...
        self.koopman_layer = Koopman_Operator(self.dim, modes = modes)
...
class AK_Projection(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.patch_size = config.patch_size 
        self.out_channels, self.hidden_size = config.out_channels, config.latent_channels 
        ...
        self.change_T_out = nn.Conv3d(
            in_channels=self.resolution[0]//self.patch_size[0] + self.resolution[0] % self.patch_size[0], # 3
        ...
        self.projection = nn.ConvTranspose3d(
            in_channels=self.hidden_size, # 27
            out_channels=self.out_channels, # 1
            kernel_size=(self.T_out, self.patch_size[1], self.patch_size[2]), # (2, 4, 4)
            stride=(self.T_out, self.patch_size[1], self.patch_size[2]), # (2, 4, 4)
            # padding=(3, 0, 0),
    ...
    def forward(self, hidden_states, dims): 
...
class AK_DecoderStage(nn.Module):
    ...
    def __init__(
        ...
        if upsample is not None: # upsample in every layer except last one
            ...
            self.upsample = upsample(config, input_resolution, dim=dim, norm_layer=nn.LayerNorm)
...
class AK_Decoder(nn.Module):
        ...
        self.stages = nn.ModuleList(
            [
                AK_DecoderStage(
                    ...
                    upsample=AK_Unmerging if i > 0 else None, # Upsample between stages (not after last one)
                    # upsampled_size=(
    ...
    def forward(
        ...
        for i, stage in enumerate(self.stages):
            ...
            hidden_states = stage_outputs[0] # shape: B, C, T, H. W : 8, 54, 3, 32, 32 ; update >>>>> B, T*H*W, C
            hidden_states_before_upsampling = stage_outputs[1] # shape: B, C, T, H. W : 8, 108, 3, 16, 16
            output_dimensions = stage_outputs[2]
...
class AK3D(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        prediction = self.projection(sequence_output, embedd_dimensions) # out: B, c_OUT, T_out, H, W ?
