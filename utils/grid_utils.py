...
def twod_meshgrid_3d(shape: List[int], device: torch.device) -> Tensor:
    """Creates 2D meshgrid feature
    ...
    Tensor
        Meshgrid tensor
    """
    bsize, size_t, size_x, size_y = shape[0], shape[1], shape[3], shape[4]
    grid_x = torch.linspace(0, 1, size_x, dtype=torch.float32, device=device)
    grid_y = torch.linspace(0, 1, size_y, dtype=torch.float32, device=device)
    grid_x, grid_y = torch.meshgrid(grid_x, grid_y, indexing="ij")
    grid_x = grid_x.unsqueeze(0).unsqueeze(0).repeat(bsize, size_t, 1, 1, 1)
    grid_y = grid_y.unsqueeze(0).unsqueeze(0).repeat(bsize, size_t, 1, 1, 1)
    return torch.cat((grid_x, grid_y), dim=2)
