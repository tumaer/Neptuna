...
class RMSE_Ch(LossComponent):
    ...
    def __init__(
        ...
        reduction: str = 'mean',
        normalization: Literal['none', 'range', 'variance', 'std', 'norm', 'root_norm', 'norm_alt'] = 'none',
        epsilon: float = 1e-8
    ):
        super().__init__(
            weight=weight,
    ...
    def forward(
        ...
        for start, end in zip(slices[:-1], slices[1:]):
            pred_g = predictions[:, :, start:end]
            label_g = labels[:, :, start:end]

            error = (pred_g - label_g) ** 2
            scale = (label_g - torch.zeros_like(label_g)) ** 2
